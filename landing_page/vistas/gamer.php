...
if (isset($_GET['add_cart']) && $sesion_activa && $rol_usuario === 'cliente') {
  ...
  echo "<script>alert('✅ Producto agregado al carrito'); window.location.href='gamer.php';</script>";
  exit;
}

// 🔹 Obtener productos de la categoría "Gamer"
$query = $conexion->query("
  SELECT p.id_producto, p.nombre_producto, p.precio, p.descripcion, p.imagen, p.stock 
  FROM productos p
  INNER JOIN categorias c ON p.id_categoria = c.id_categoria
  WHERE c.nombre_categoria = 'Gamer'
");
...
<head>
  ...
  <title>Gamer - Electro Wizard</title>
...
<body class="bg-gray-100 font-sans">
  ...
  <header class="bg-[#0b1c35] text-white shadow-md fixed top-0 w-full z-50">
    <div class="max-w-7xl mx-auto flex justify-between items-center px-6 py-4">
      ...
      <nav class="flex items-center space-x-4 text-white font-medium fade-up">
        <span class="text-sm md:text-base">Explora nuestro mundo <strong class="text-[#00e0ff]">Gamer</strong> 🎮</span>
  ...
  <main class="pt-32 pb-16 max-w-7xl mx-auto px-6 fade-up">
    <h2 class="text-3xl font-bold text-[#0b1c35] mb-8 text-center">Zona Gamer - Electro Wizard</h2>
        ...
        <?php foreach ($productos as $p): ?>
          <div class="border rounded-xl shadow hover:shadow-xl transition hover:-translate-y-1 bg-white p-5 text-center">
            ...
            <?php elseif($rol_usuario === 'cliente'): ?>
              <a href="gamer.php?add_cart=<?php echo $p['id_producto']; ?>" class="btn-electric mt-2 inline-block text-center">Agregar al carrito</a>
            <?php else: ?>
    ...
    <?php else: ?>
      <p class="text-center text-gray-500 mt-10">No hay productos disponibles en esta categoría por el momento.</p>
    <?php endif; ?>
  </main>
