...
// 🛒 Agregar al carrito (igual que en index.php)
if (isset($_GET['add_cart']) && $sesion_activa && $rol_usuario === 'cliente') {
  ...
  echo "<script>alert('✅ Producto agregado al carrito'); window.location.href='televisores.php';</script>";
  exit;
}

// 🔹 Traer productos de categoría 'Televisores'
$query = $conexion->query("
  SELECT p.id_producto, p.nombre_producto, p.precio, p.descripcion, p.imagen, p.stock 
  FROM productos p
  INNER JOIN categorias c ON p.id_categoria = c.id_categoria
  WHERE c.nombre_categoria = 'Televisores'
");
$televisores = $query->fetch_all(MYSQLI_ASSOC);
?>
<!DOCTYPE html>
<html lang="es">
<head>
  ...
  <title>Televisores - Electro Wizard</title>
...
<body class="bg-gray-100 font-sans">
  ...
  <header class="bg-[#0b1c35] text-white shadow-md fixed top-0 w-full z-50">
    <div class="max-w-7xl mx-auto flex justify-between items-center px-6 py-4">
      ...
      <nav class="flex items-center space-x-4 text-white font-medium fade-up">
        <span class="text-sm md:text-base">Aquí verás todos nuestros <strong class="text-[#00e0ff]">Televisores</strong> 📺</span>
  ...
  <main class="pt-32 pb-16 max-w-7xl mx-auto px-6 fade-up">
    <h2 class="text-3xl font-bold text-[#0b1c35] mb-8 text-center">Televisores Disponibles</h2>

    <?php if (count($televisores) > 0): ?>
      <div class="grid md:grid-cols-4 sm:grid-cols-2 gap-8">
        <?php foreach ($televisores as $tv): ?>
          <div class="border rounded-xl shadow hover:shadow-xl transition hover:-translate-y-1 bg-white p-5 text-center">
            <img src="../../dashboard_admin/imagenes_productos/<?php echo htmlspecialchars($tv['imagen']); ?>" 
                 alt="<?php echo htmlspecialchars($tv['nombre_producto']); ?>" 
                 class="w-40 h-40 object-contain mx-auto mb-4">
            
            <h3 class="text-lg font-semibold text-[#0b1c35] mb-2">
              <?php echo htmlspecialchars($tv['nombre_producto']); ?>
            </h3>
            <p class="text-gray-600 text-sm mb-2"><?php echo htmlspecialchars($tv['descripcion']); ?></p>
            <p class="text-[#0b1c35] font-bold mb-2">$<?php echo number_format($tv['precio'], 0, ',', '.'); ?> COP</p>
            <p class="text-sm text-gray-500 mb-3">Stock disponible: <?php echo intval($tv['stock']); ?></p>
            ...
            <?php elseif($rol_usuario === 'cliente'): ?>
              <a href="televisores.php?add_cart=<?php echo $tv['id_producto']; ?>" class="btn-electric mt-2 inline-block text-center">Agregar al carrito</a>
            <?php else: ?>
    ...
    <?php else: ?>
      <p class="text-center text-gray-500 mt-10">No hay televisores disponibles en este momento.</p>
    <?php endif; ?>
  </main>
